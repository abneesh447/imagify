...
const BuyCredit = () => {
  ...
  const initPay = async (order) => {
    const options = {
      ...
      handler : async (response)=>{
        ...
        try {
          const {data} = await axios.post(backendUrl+ "/api/v1/users/verify-razor"
  ...
  const paymentRazorpay = async (planId) => {
    try {
        ...
        const { data } = await axios.post(
            backendUrl + "/api/v1/users/pay-razor",
            { planId },
            { headers: { token } }
        );
        ...
        if (data.success) {
            console.log(' Payment Success, opening Razorpay...');
            initPay(data.order);
        } else {
  ...
  return (
    ...
    className='min-h-[80vh] text-center pt-14 mb-10'>
      <button className='border border-gary-400 px-10 py-2
      rounded-full mb-6'>Our Plans</button>
      <h1 className='text-center text-3xl font-medium mb-6 sm:mb-10'>Choose the plan</h1>
      <div className='flex flex-wrap gap-5 text-left items-center justify-center'>
        {
          plans.map((item,idx)=>(
            <div key={idx} className='bg-white drop-shadow-sm rounded-lg py-12
